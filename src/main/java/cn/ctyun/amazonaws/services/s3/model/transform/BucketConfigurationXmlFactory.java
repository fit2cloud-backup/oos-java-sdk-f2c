...
public class BucketConfigurationXmlFactory {
    ...
    public byte[] convertToXmlByteArray(BucketLoggingConfiguration loggingConfiguration) {
        ...
        if (loggingConfiguration.isLoggingEnabled()) {
            ...
            if (loggingConfiguration.getDestinationBucketName() != null) {
            ...
            if (loggingConfiguration.getCtyunTriggerDestinationBucketName() != null) {
                xml.start("TriggerTargetBucket").value(loggingConfiguration.getCtyunTriggerDestinationBucketName())
                        .end();
                xml.start("TriggerTargetPrefix").value(loggingConfiguration.getCtyunTriggerDestinationLogFilePrefix())
                        .end();
            }
            if (loggingConfiguration.getCtyunTriggerSourceBucketName() != null) {
                xml.start("TriggerSourceBucket").value(loggingConfiguration.getCtyunTriggerSourceBucketName()).end();
                xml.start("TriggerSourcePrefix").value(loggingConfiguration.getCtyunTriggerSourceLogFilePrefix()).end();
            }
            xml.end();
        }
        xml.end();
    ...
    public byte[] convertToXmlByteArray(CtyunBucketTriggerConfiguration triggerConfiguration) {
        XmlWriter xml = new XmlWriter();
        xml.start("TriggerConfiguration", "xmlns", Constants.XML_NAMESPACE);
        xml.start("Trigger");
        CtyunTriggerMetadata trigger = triggerConfiguration.getTriggers().get(0);
        if (trigger.getTriggerName() != null) {
            xml.start("TriggerName").value(trigger.getTriggerName()).end();
        }
        xml.start("IsDefault").value(String.valueOf(trigger.isDefault())).end();
        if (trigger.getRemoteSites() != null) {
            for (CtyunRemoteSite s : trigger.getRemoteSites()) {
                xml.start("RemoteSite");
                xml.start("RemontEndPoint").value(s.getRemoteEndPoint()).end();
                xml.start("ReplicaMode").value(s.getReplicaMode()).end();
                xml.start("RemoteBucketName").value(s.getRemoteBucketName()).end();
                xml.start("RemoteAK").value(s.getRemoteAK()).end();
                xml.start("RemoteSK").value(s.getRemoteSK()).end();
                xml.end();
            }
        }
        xml.end();
